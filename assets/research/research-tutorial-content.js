...
const intro = [
  ...
  page('intro-connections', 'intro', '03 · 两种连线', '关系线与导线',
    '底栏只记忆下一次 Alt 拖拽要创建的连线种类；普通左键始终保留为选择和移动。', [
      { title: '关系线', body: '选择“关系线”，按住 Alt 从一个节点主体拖到另一个节点主体。起点已在多选中时，会从全部已选节点同时连向同一目标；它没有方向和端口类型，不参与计算。' },
      { title: '导线', body: '选择“导线”，按住 Alt 从真实端口拖到兼容端口。多选起线要求全部已选节点具有同名、同方向端口；从输入端反向起线可把一个输出扇出到多个输入，值输入只允许一条来源，Pulse 输入允许多个来源。' },
      { title: '红色预览', body: '红色活动线表示通道、类型、位宽、占用或多选成员冲突；多重连线中任一候选非法时整组不写入模型或历史。只有成功建立的导线才算依赖。' },
    ]),

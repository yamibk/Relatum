...
| `assets/research.html`、`assets/research/` | 起步页“研究”的同源透明 iframe 与独立 M1 内存画布。`research-workspace.js` 只向父页提供 `activate` / `suspend` / `dispose`；`research-editor.js` 协调无 DOM 模型与投影；`research-model.js` 维护节点/边 ID、邻接索引和约 50 步历史；`research-canvas.js` 提供 DOM 节点、Canvas2D 静态连线、活动 SVG、512 单位连线空间网格、选择、相机与小地图。当前不加载 WebGL，不访问后端 API、存储或用户数据。 |
...
- `research.html` 承载透明、无可见说明/占位/工具文字的单画布；`research-workspace.js` 只向父页公开 `activate`、`suspend`、`dispose`，并在激活时导入独立研究运行时。输入监听、ResizeObserver 和绘制 RAF 只在激活期存在，离页暂停；iframe 复用期间内存节点状态保留，销毁后释放。
- M1 支持空白双击/N 新建、中文文本编辑、Tab 子节点、Enter 同级节点、Alt 拖连线、单选/多选/框选/多选移动、删除、撤销重做、定点缩放、空格/中键平移、键盘平移、适应内容和小地图。数据只在 `research-model.js` 的内存模型中，连线保留 `role`、`fromPort`、`toPort` 显式字段；不得用 DOM 几何反写第二份业务状态。
...
- 契约验证：`node tests/start-research-shell-contract.js`、`node tests/research-canvas-baseline-contract.js`、`node tests/research-fork-baseline-contract.js`。
...
研究工作区改动：`node tests/start-research-shell-contract.js`、`node tests/research-canvas-baseline-contract.js`、`node tests/research-fork-baseline-contract.js`，并运行原画布启动、性能、历史、样式、尺寸和 Markdown 契约。研究壳契约确认首屏零研究资源、首次进入按需挂载、生命周期、失败回退及无后端/API；画布契约确认独立模型、显式端口、索引、历史、DOM/Canvas2D/SVG 边界和零存储依赖；分叉契约确认原编辑器不反向引用研究代码。桌面共享关闭链另运行 `node tests/note-focus-browser.js` 与原有桌面 Python 回归；仅研究内存画布无需重新打包发行版。

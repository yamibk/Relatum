...
| `assets/note-live-editor.js` | 基于离线 CodeMirror 6 / Lezer 的源码保真 Markdown Live Preview；负责分帧补齐当前视口语法树、可见富块增量重扫、Obsidian 式源码标记染色、围栏代码语言高亮与语言标签复制、当前语法单元显露、Callout/任务/双链/图片/行内与块公式/Mermaid/表格/derive 组件、IME、括号自动闭合、最高优先级空引用退出、加粗/围栏代码等快捷键与编辑器快照接口，并向阅读模式提供同一安全 Markdown/本地资源渲染入口。IME 以 CodeMirror 原生 composition 状态为准，由 `idle / composing / settling` 单一会话协调：候选输入期间只映射已有投影，禁止重新解析和保存预编辑拼音，结束后只重扫一次可见区；同步快照始终返回最后已提交文档，工作区保存和切页通过 `whenInputSettled()` 等待最终候选。笔记图片在 Live Preview 中始终保持可视投影，独占行图片贴齐正文左缘；点击选中隐藏源码范围，右下角手柄等比例缩放并在松手时用单次 CodeMirror 事务写回 Obsidian 像素宽度语法。图片与未激活富块、行内图片/公式使用原子光标范围；点击渲染公式会显露源码并把空光标放进定界符内。独占行本地图片还可在显式“图片文字”模式中创建、编辑和拖动多个纯文字框；图片文字 textarea 自己等待 compositionend，候选窗触发 blur 时不得提交拉丁预编辑串；若明确点击图片内外（同一图片上的另一个文字框除外），则在指针按下时同步冻结当时可见值（如 `t`）并立即隐藏 textarea/工具栏，但保留原生 textarea 到 compositionend 后再提交，避免 WebView2 丢失预编辑值；编辑中单击另一个文字框改为同步提交当前文字、保持图片文字模式与工具栏，并在提交后通过当前适配器按 ID 定位被点文字框，直接进入编辑且光标放在末尾，提交事务自身的重钉选中不得把它抢回原框；组合输入中该次提交延后到 compositionend，且已排定的提交不得被 `cancelDraft()` 丢弃；`Esc` 取消候选不得把它重新保存。显式打开的图片文字工具栏会固定绑定当前图片：焦点变化或富块重绘导致的短暂选区漂移只恢复选区，不得自动关闭。编辑态由同样式隐藏文字镜像实时测量宽高，展示态与编辑态必须保持同一中心、字号、行高和内容尺寸；输入、改字号、拖动和键盘微调都把完整文字框约束在图片范围内。选中文字后直接打字或粘贴在末尾追加并进入编辑，原生输入及事务保护不得让正文选区覆盖隐藏图片源码；清空已有框提交会删除对应元数据，新建空框不保存，Esc 取消编辑；选中态 Delete/Backspace 删除整框并保留撤销，最后一框删除移除完整注释，撤销/重做需同步图片范围。选中文字后支持 `Enter` 编辑、方向键 1px 微调和 `Shift+方向键` 10px 微调。平时文字层只显示不命中，每个视图共用一个 `ResizeObserver` 同步比例字号，每次内容/位置/样式修改只提交一次事务。只有源码模式显示完整原语法。实时预览与源码模式通过 CodeMirror `Compartment` 原位重配置，不能重建文档状态或清空撤销历史。Markdown 字符串始终是唯一真源，原始 HTML/SVG 只显示源码。 |

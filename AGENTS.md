...
| `trash.html` | 回收站管理页。 |
| `assets/start.js` | 起步页状态、顶层“画布 / 笔记 / 研究 / 生涯”工作区切换、最近/分组/收藏（列表数字归类、方向键、退格、回车、搜索及画布帮助快捷键仅在顶层画布工作区响应；速记墙全局键盘入口同样检查顶层工作区，避免隐藏页面抢占笔记/生涯输入）、页面切换、主题/背景/翻页速度，以及学习/树状/速记三页的可选前台计时；“研究”当前只是始终可见的纯空白重建位，没有运行时、数据接口或保存链。顶层工作区切换会清理中断的旧动画状态，并以方向感明确的前后双层交接保持快速连续切换稳定；进入或离开全屏生涯工作区时按切换前后的真实舞台顶边补偿离场页，避免顶栏退出/进入文档流造成纵向跳帧。齿轮面板还用 `canvas:darkCardPresentation:v1` 分别保存树状页与学习页深色彩色卡片的底色、边框/侧条强度和“智能/白/黑”字体颜色，两页均默认底色 15%、边框/侧条 100%、白字，浅色模式只禁用控件而不改值。笔记和生涯运行时均按需加载，首屏稳定后的空闲阶段会预载生涯轻量运行时与冻结快照，首次切换通常直接显示结果。活跃页、学习进度视图与专注每日任务视图在内部错峰入场未完成时离页，会先冻结当前内部动画帧，等外层退场隐藏后再清理，避免元素瞬间补齐。 |
...
- 首页是书本式工作台，不是营销页。
- 顶栏“画布 / 笔记 / 研究 / 生涯”只切换现有起步页壳中的直属工作区面板；画布工作区 DOM 不卸载，所以当前特殊页、分组、书脊位置和滚动状态必须保留。切换使用同一 `--start-turn-*` 速度变量与低动态降级；研究面板保持纯空白且不加载任何运行时，笔记和生涯保持既有按需加载/空闲预热。离开笔记前必须等待保存链；按钮选择器只能命中 `button[data-start-workspace]`，不能把带当前状态的 `body[data-start-workspace]` 误绑为点击入口。四个顶层入口支持方向键与 Home/End 切换。
...
- 顶栏保留“研究”入口和四栏切换顺序，`assets/index.html` 中的直属面板必须保持为空。
- 当前没有研究运行时、数据目录、HTTP API、桌面桥接、保存链或旧格式兼容；后续重建不得复用已删除实现的协议假设。
- 契约验证：`node tests/start-research-placeholder-contract.js`。
...
研究空白入口改动：`node tests/start-research-placeholder-contract.js`。该契约确认四栏入口与空面板保留，同时旧运行时、API、偏好和桌面桥接没有回流。桌面共享关闭链另运行 `node tests/note-focus-browser.js` 与原有桌面 Python 回归；不需要重新打包发行版。

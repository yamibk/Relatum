...
| `assets/editor.js` | 编辑器页面编排：加载/保存、模式切换、模板、导出、背景、AI/图谱入口和参考画布父页协调；未使用附件清理先显示中英文自定义确认浮窗，明确删除范围与不可撤销提示，支持 Esc、遮罩取消和键盘焦点闭环，执行期间锁定操作。主画布坐标模式开启时由本机偏好临时接管辅助底纹：保留全局底纹选择但隐藏视口/顶栏底纹并禁用背景面板入口，关闭坐标后原样恢复，不写背景 API。 |

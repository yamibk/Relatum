...
| `assets/research/workspace.js`、`workspace.css` | 按需研究入口与独立纯白/中性黑样式；单项目、变量/记录/公式属性、对象目录/对象表、普通关系编辑、定位/适应视野、输入收尾、工作区与桌面关闭保存，使用现有语言服务。 |
...
- 研究：`/api/research/projects`、`/api/research/project?id=project-main`；返回项目与字节指纹，读取不创建数据；只接受受管项目 ID。
...
- 单项目支持变量、自由记录、公式草稿、画布与对象表。修改属性或源文同步到同一对象的各处呈现；添加引用不复制对象，从视图移除不删除对象。创建、属性编辑、拖动、添加/移除引用统一进入会话撤销；手势每帧只预览，松手一次提交，取消恢复原位。创建类型栏选择记录/变量/公式后左键双击空白放置，初始记录；顶栏“＋”仍直接创建。双击节点或画布内 Shift+Enter 编辑，根节点 Enter 也编辑；有父级的节点 Enter 建同级，Tab 建子分支。表单不接管分支快捷键。新呈现优先找可见空位、空间不足时错位放置，选中节点临时置前，不改持久化叠放顺序。长期键鼠操作逐步向旧画布编辑器靠近。

...
(async () => {
  ...
  try {
    ...
    await box('a').dblclick();
    await input.fill('Right click saved');
    await input.click({ button: 'right' });
    await page.waitForFunction(() => !editor.inputPending);
    assert.equal(await input.count(), 0, 'right-click exits editing');
    assert.equal((await items())[0].text, 'Right click saved');
    assert.equal(await box('a').evaluate((label) => label.classList.contains('is-selected')), true);
    assert.equal(await page.locator('.note-live-image-frame.is-image-text-mode').count(), 1);
    await page.keyboard.press('Enter');
    await input.fill('IME ');
    await cdp.send('Input.imeSetComposition', { text: 't', selectionStart: 1, selectionEnd: 1 });
    await input.click({ button: 'right' });
    await page.waitForFunction(() => !editor.inputPending);
    assert.equal(await input.count(), 0, 'native IME right-click settles and exits');
    assert.equal((await items())[0].text, 'IME t');
    await page.keyboard.press('Enter');
    await input.fill('');
    await input.click({ button: 'right' });
    await page.waitForFunction(() => !editor.inputPending);

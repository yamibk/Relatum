...
| `assets/study-goal-tree.js`、`assets/study-route.js` | 学习页“目标树”V4 的无 DOM 链接模型与交互层；阶段用 `contains` 汇总内部任务，任务/阶段用 `requires` 解锁后续，多个入站依赖按 AND 判定。主链接驱动双向自由布局和整棵子树拖动，附加依赖不影响排版；还负责阻塞原因、“下一步”定位、阶段折叠和每树独立镜头。阶段右键直接弹调色盘，任务节点右键菜单含“颜色”项，颜色都写任务或阶段节点的 `color` 字段。 |

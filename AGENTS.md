...
| `assets/research.html`、`assets/research/`、`research_store.py` | 起步页“研究”的同源透明 iframe、独立研究画布与 V2 可组合运行时。`research-workspace.js` 只提供 `activate` / `suspend` / `dispose`；`researchVersion:2` 的节点保存 `type`、`config`、`statePolicy` 与可选 `savedState`，边显式分为不执行的 `relation` 和带端口的 `wire`。`research-node-definitions.json` 是前端建节点/属性控件/端口和 Python 端口白名单的共同声明源；V1 不迁移，读取后显示空白 V2，首次保存只覆盖研究文件。稳定值为 number/boolean/string/time/Bits<1–64>，Pulse 只在运行时存在；组合值自动沿脏下游传播，状态批次统一读取旧状态后提交，Register 可作为合法反馈边界，组合环报错。节点族包括 Note、Constant、Button、Toggle、Current Time、Clock、Math、Logic、Compare、Select、Convert、Bits、Register、Counter、Edge Detector、统一 Timer、Monitor、Probe 和 Lamp；Probe 用可选 `value` 和多来源 `pulse` 输入记录有界运行轨迹，条目包含仿真时间、Pulse 序号、来源和值，不持久化、不进历史。旧四则一级按钮、倒/正计时、Delay 与 Result 已删除。底栏提供选择/添加/关系线/导线和 Run/Pause/Step/Reset/倍率；关系线裁切到节点真实边框，导线跟随真实 DOM 高度和端口中心；输入、输出端口都能起线，反向拖拽自动规范为输出到输入，非法候选只显示红色活动预览且不落模型。从已占用值输入主动拖出可保留 edge ID 原子重接，事件输入继续新增来源，从输出落到已占用值输入仍拒绝。添加面板可搜索分组，选中节点用右侧属性面板编辑真实配置，选中 Probe 还显示运行轨迹；标题不改变语义。右缘有 32px 显式热区唤出数字页栏。每页模型、历史、镜头、仿真时钟、事件队列、Probe 轨迹和运行态隔离；离页/隐藏/销毁取消带修订令牌的调度，返回恢复运行意图。手动 Button/Toggle 事件后必须刷新运行调度，以便正在运行的页能继续新启动的 Timer。状态默认不保存，只有节点显式 `persist` 才保存离散状态；投影、Probe 轨迹、队列、缓存、统计、历史和句柄不落盘。画布继续使用 DOM 节点、Canvas2D 静态连线、活动 SVG、512 单位网格、几何缓存和小地图索引，不加载 WebGL。`GET/POST /api/research/workspace` 只写 `data/research-workspace/workspace.json`，保留原子写入、SHA-256 冲突、有效 V2 备份与损坏隔离。图分析已顺延：未来必须通过显式 Graph Snapshot/节点引用输入读取指定关系线、导线或子图，不得偷偷依赖整页；动作节点与子电路也仍未实现。 |
...
- `research.html` 承载透明单画布、分页栏、紧凑工具/运行栏、搜索添加面板、右侧属性面板、保存状态和右下角说明；`research-workspace.js` 只向父页公开 `activate`、`suspend`、`dispose`，并在激活时导入独立运行时、等待 `/api/research/workspace` 读取。输入监听、ResizeObserver、绘制/计算帧与仿真句柄只在激活期存在；隐藏、离页和销毁必须用修订令牌取消过期回调，销毁保存用 keepalive 收尾。
- `researchVersion:2` 不兼容 V1：非 V2 研究文件读取为空白 V2，首次保存直接覆盖且不备份 V1。节点为 `{id,type,label,x,y,width,height,config,statePolicy,savedState?}`；边只能是 `relation` 或显式端口 `wire`。关系线不执行；值输入单连接、事件输入多来源、输出可扇出。建线时即时拒绝错误方向、value/event 通道、声明类型、已知 Bits 位宽、同组输入不匹配和占用，不得先写历史再回滚；拖拽中的非法候选只显示红色活动线，松手不入模型。从已占用值输入发起的合法重接是唯一占用例外：校验时忽略原 edge，提交时保留其 ID 并只写一条历史。
...
- 节点族固定为 Note、Constant、Button、Toggle、Current Time、Clock、Math、Logic、Compare、Select、Convert、Bits、Register、Counter、Edge Detector、Timer、Monitor、Lamp。Timer 统一正/倒计时并由 start/pause/reset 事件控制；旧四则一级按钮、倒/正计时、Delay 和 Result 不得恢复。底栏提供选择/添加/关系线/导线与 Run/Pause/Step/Reset/倍率；Alt 拖关系线入口已删除。关系线必须落在渲染后节点边框，导线必须落在真实端口中心；输入与输出均可起线，反向手势只调整交互方向，不改变持久 wire 的输出到输入语义。标题只改 label，真实行为只在属性面板修改 config。
...
- 研究正文只保存到 `data/research-workspace/workspace.json`，有效 V2 备份为同目录 `workspace.backup.json`；损坏主文件先隔离为 `workspace.corrupt-时间.json`，再恢复备份或空白文档。前端写入携带读取时的 SHA-256 修订号，409 冲突不得覆盖；服务端保存前必须读取同一节点声明并严格校验，再沿用 `_atomic_write_json`。事件队列、投影、编译缓存、性能统计、历史、工具状态和调度句柄不得进入文档。
- 图分析、动作节点和子电路仍未实现；Probe 有界值/事件轨迹已完成。下一阶段先设计类型化子电路定义/实例、内部状态隔离和修订语义，再实现。未来图分析只能通过显式 Graph Snapshot/节点引用输入读取关系线、导线或指定子图，禁止节点偷偷依赖整页。契约验证：`node tests/start-research-shell-contract.js`、`node tests/research-canvas-baseline-contract.js`、`node tests/research-pages-regression.js`、`node tests/research-compute-regression.js`、`node tests/research-runtime-regression.js`、`node tests/research-persistence-contract.js`、`python -m unittest tests.test_research_store`、`node tests/research-fork-baseline-contract.js`。可选真实浏览器验收为 `node tests/research-circuit-browser.js <本地服务 URL> [report.json]`，夹具拦截研究读写，不接触真实研究数据。
...
研究工作区改动：`node tests/start-research-shell-contract.js`、`node tests/research-canvas-baseline-contract.js`、`node tests/research-pages-regression.js`、`node tests/research-compute-regression.js`、`node tests/research-fork-baseline-contract.js`，并运行原画布启动、性能、历史、样式、尺寸和 Markdown 契约。研究壳契约确认首屏零研究资源、首次进入按需挂载、生命周期、失败回退、无整页原生标题提示及无后端/API；画布契约确认独立模型、显式端口、索引、历史、DOM/Canvas2D/SVG 交接边界和零存储依赖；分页回归确认第 1 页保护、追加/删除顺序、逐页模型/历史/镜头隔离与空白页回退；计算回归确认独立版本 Schema、严格类型值、显式端口、`5 + 6 → 11`、视觉线隔离、节点级错误、模型不变性及 1503 节点迭代执行；分叉契约确认原编辑器不反向引用研究代码。桌面共享关闭链另运行 `node tests/note-focus-browser.js` 与原有桌面 Python 回归；仅研究内存画布无需重新打包发行版。

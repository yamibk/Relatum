...
- `research/` 是新增研究工作区的用户数据；实验期格式调整只限本轮明确选定的研究项目，不得连带处理原画布、笔记和生涯。构建覆盖保护与 `.gitignore` 同样包含此目录。
...
| `assets/research/workspace.js`、`workspace.css` | 按需研究入口与独立纯白/中性黑样式；多项目编号栏、创建/改名、安全切换与会话隔离；变量/记录/公式属性、对象目录/对象表、普通关系编辑、定位/适应视野、输入收尾、工作区与桌面关闭保存，使用现有语言服务。 |
...
| `assets/research/core/tree.js` | 无 DOM 的视图树校验、迭代后代/隐藏遍历与右向有界布局；`parentId/collapsed` 只组织呈现，不改变对象关系；配合模型做整枝移动、折叠、展开祖先与脱离父级。 |
...
| `trash.html` | 回收站管理页。 |
| `assets/start.js` | 起步页状态、顶层“画布 / 笔记 / 研究 / 生涯”工作区切换、最近/分组/收藏（列表数字归类、方向键、退格、回车、搜索及画布帮助快捷键仅在顶层画布工作区响应；速记墙全局键盘入口同样检查顶层工作区，避免隐藏页面抢占笔记/生涯输入）、页面切换、主题/背景/翻页速度，以及学习/树状/速记三页的可选前台计时；顶层工作区切换会清理中断的旧动画状态，并以方向感明确的前后双层交接保持快速连续切换稳定；进入或离开全屏生涯工作区时按切换前后的真实舞台顶边补偿离场页，避免顶栏退出/进入文档流造成纵向跳帧。齿轮面板还用 `canvas:darkCardPresentation:v1` 分别保存树状页与学习页深色彩色卡片的底色、边框/侧条强度和“智能/白/黑”字体颜色，两页均默认底色 15%、边框/侧条 100%、白字，浅色模式只禁用控件而不改值。齿轮面板的“禁用研究入口”出厂默认开启，用本机偏好 `canvas:researchEntryDisabled` 控制顶栏“研究”标签：只有显式存过 `'0'` 才放开，并且只在用户切换开关时写盘（开机应用与面板“恢复默认”都不写，避免把“没碰过”记成“碰过”）；开启后该标签直接从前端消失，`index.html` 首帧就写根节点 `data-research-entry-disabled` 并把上次停留的研究页回退成画布，`styles.css` 同步把四栏收成三栏（窄屏同理、滑块宽度改 `(100% - 8px) / 3`、生涯只前进两档），研究页尚未离开期间滑块收成不选中；按钮同时设原生 `disabled`，点击、方向键（`Home`/`End` 按实际可用项取首尾）与 `RelatumStartWorkspace.set()` 由 `setStartWorkspace` 统一拦截（没有放行分支），用户开启开关时若正停在研究页由 `applyResearchEntryDisabled` 自动退回画布；面板“恢复默认”清掉该偏好并回到出厂默认（禁用）。笔记和生涯运行时均按需加载，首屏稳定后的空闲阶段会预载生涯轻量运行时与冻结快照，首次切换通常直接显示结果。活跃页、学习进度视图与专注每日任务视图在内部错峰入场未完成时离页，会先冻结当前内部动画帧，等外层退场隐藏后再清理，避免元素瞬间补齐。 |
...
- 研究：`/api/research/projects`、`/api/research/project?id=<projectId>`；列表返回 ID/标题/创建时间或错误，项目读取返回快照与字节指纹，读取不创建数据；只接受受管项目 ID。
...
- 研究：`/api/research/create`、`/api/research/save`。保存携带 `requestId / expectedRevision / expectedFingerprint / project`，复用应用跨进程锁和原子文本写入；校验回环 Host、同源 Origin 和 JSON Content-Type。冲突 409 时保留恢复副本，普通失败保留前端草稿与重试入口；不执行项目内容。
...
- 首页是书本式工作台，不是营销页。
- 顶栏“画布 / 笔记 / 研究 / 生涯”只切换现有起步页壳中的直属工作区面板；画布工作区 DOM 不卸载，所以当前特殊页、分组、书脊位置和滚动状态必须保留。切换使用同一 `--start-turn-*` 速度变量与低动态降级；研究 ES modules/CSS 只在首次进入加载，笔记和生涯保持既有按需加载/空闲预热。离开笔记或研究前必须等待各自保存链，按钮选择器只能命中 `button[data-start-workspace]`，不能把带当前状态的 `body[data-start-workspace]` 误绑为点击入口。四个顶层入口支持方向键与 Home/End 切换。
...
- `core/session-cache.js` 只缓存已保存的非当前项目，按最近使用顺序最多保留 6 项、合计 64 MiB 序列化估算量（包含文档、撤销和重做；不是进程实际内存硬限）。超限释放旧会话，当前/未保存文档不参与淘汰；被淘汰项目从磁盘重开并重置会话撤销/镜头。释放工作区时清空模型引用、字段历史、节点 DOM、几何缓存、观察器与监听器并释放编辑锁。浏览器多项目回归包含 12 项目/108 次切换编辑压力测试，强制 GC 后检查堆、DOM 和监听器不持续增长及锁归零。

- 右侧保留窄槽编号栏，悬停/聚焦展开，点击箭头可固定展开，外点收起；方向键/Home/End 导航，超长列表内部滚动且 `＋` 固定可见。`＋` 显式创建“研究项目 N”，标题旁铅笔原位改名，Enter/失焦提交、Esc 取消，等待中文候选，空白不提交、最多 500 字符、允许重名；改名走模型命令与保存链，可撤销，不改变编号。本轮不提供项目删除/回收站。
- 创建接口接受 `{projectId,title}`，前端在响应不确定时复用 ID 重试；同 ID 已存在时只读返回，不能覆盖。旧空请求保留 `project-main` 行为。`createdAt` 由后端生成并在保存时保留，旧项目不自动补时间。`relatum:research:lastProject:v1` 只记本机最近成功打开的 ID，不存在时回退列表首个可读项目。
- 新建/切换先等输入结束并排空当前保存队列；失败留在原项目。加载目标前获取目标 Web Lock，读取校验成功才释放原锁并替换会话。项目模型/撤销、视图、镜头按 ID 留在内存，切回时磁盘指纹相同才复用历史，外部变化则新建模型；字段历史/选中/连线草稿清空，迟到内容预览取消。只读项目可切走再切回尝试获取编辑权。切换期间禁用重复操作，离工作区/桌面关闭等待同一在途链；仅当前场景渲染。镜头/撤销不跨应用重启保存。

- 对象“永久删除”位于当前对象标题旁和对象表每行；用户明确要求点击即执行、无二次确认、不可撤销。先等待输入/保存队列，再调用 `/api/research/delete-object`（同 create/save 的同源 JSON 与跨进程锁保护，参数为 projectId/objectId/expectedRevision/expectedFingerprint）；后端校验修订/指纹，物理改写项目与含该对象的恢复副本，清理对象、所有呈现和端点关系，不生成删除备份。子对象保留，直接子级变根。成功清空项目会话撤销/重做和字段历史；响应不确定时锁编辑并提供重试删除，幂等重试安全。项目 `_deletedObjectIds` 只保留 ID，普通保存不得移除标记或重新引入删除对象，拒绝这类快照时不写恢复副本。“从视图移除”仍可撤销，不能替代永久删除。应用不承诺磁盘扇区擦除或清理用户导出/系统备份。研究 Python、模型队列与连线浏览器回归覆盖永久删除、恢复副本、旧快照拒绝、响应丢失重试和只读禁用。

- 每个项目支持变量、自由记录、公式草稿、画布与对象表。修改属性或源文同步到同一对象的各处呈现；添加引用不复制对象，从视图移除不删除对象。创建、属性编辑、拖动、添加/移除引用统一进入会话撤销；手势每帧只预览，松手一次提交，取消恢复原位。创建类型栏选择记录/变量/公式后左键双击空白放置，初始记录；顶栏“＋”仍直接创建。双击节点或画布内 Shift+Enter 编辑，根节点 Enter 也编辑；有父级的节点 Enter 建同级，Tab 建子分支。表单不接管分支快捷键。新呈现优先找可见空位、空间不足时错位放置，选中节点临时置前，不改持久化叠放顺序。长期键鼠操作逐步向旧画布编辑器靠近。
...
- 500ms 空闲合并自动保存，单请求串行追到最新修订；失败保留草稿，冲突不继续覆盖，支持导出当前草稿。浏览器同源配置内用 Web Locks 限制每项目单写入窗口；不同配置仍由后端修订/指纹保护。只读窗口重开或切走再切回项目可再次尝试获取写入权。
...
- 普通关系保存为 `core.association` / `typeVersion:1`，`label` 最长 2000 UTF-16 单位，`ends` 引用两个不同对象；画布可选 `links[]` 独立保存 `id/relationId/sourceId/targetId`，后端核对呈现与对象端点一致。选中卡片后点“连接节点”再点目标，Esc/空白取消；关系名称复用局部输入历史和组合输入收尾。点击线或右侧关系列表编辑名称、删除关系；移除卡片呈现只移除相关线，仍可从列表管理关系，撤销恢复呈现及线。关系删除会一并删除各画布的对应线，统一撤销。
- 连线暂用常驻 SVG 直线、透明加宽命中路径与纯文本标签，跟随同一 CSS 相机；ResizeObserver 缓存节点尺寸，拖动预览/取消与真实尺寸变化更新端点，平移不重算几何。标签只显示前 80 字符，属性保留全名。底部“适应视野”包围全部呈现，属性栏“定位到画布”居中当前对象呈现，相机不持久化。研究主体、侧栏、卡片统一 #fff / #111，无暖黄/绿灰表面或背景点阵；不影响其他工作区主题。
- Alt＋左键拖动节点连接另一对象，显示虚线预览；释放在自身/空白不创建，Esc/失焦/指针取消清理预览，保留原坐标。原“连接节点”按钮继续可用。
- `core.canvas.representations[]` 的可选 `parentId` 引用同视图呈现，`collapsed` 为布尔；前后端拒绝循环/失效父级。分支线由层级派生，不加入对象普通关系。新分支为自由记录，创建同时展开父级；拖动整枝含隐藏后代，松手只提交一次，取消恢复全枝；整理按实测卡片尺寸右向布局且不移动其他树/自由节点，不自动避碰或重排。折叠仅隐藏后代及其线，适应视野只算可见节点；可写窗口定位隐藏对象会展开祖先并进入撤销。移除中间呈现后直接子级成为根，对象及普通关系仍保留；脱离父级只清归属。隐藏节点保留 DOM，不宣称已虚拟化。
- 后续入口为 `docs/研究工作区总体规划.md` Q04 第一段：独立嵌套分区、整组移动/折叠和层级导航，随后手写；Q05 为图结构分析探针。尚未实现左右布局/固定点/重挂手势/旋转、手写或计算。常驻 DOM/SVG 与目录重绘适合当前小项目，尚无大场景基准承诺。
...
- 构建会整体替换 `Relatum-release/`；若目录内已有 `canvases/`、`notes/`、`research/` 或 `data/`，默认拒绝覆盖，除非显式 `-ForceReplaceUserData`。
...
多项目另运行 `node tests/research-projects-browser.js`，覆盖创建响应丢失/重复点击、改名取消/空白/合成 IME、切换保存失败、目标损坏、独立历史/镜头、外部修改、跨窗口锁释放/重获、重开最近项目/失效回退、键盘与 27 项长列表。三个研究浏览器脚本仅在临时浏览器上下文显式开启研究入口，不修改用户的 `canvas:researchEntryDisabled` 偏好。

普通连线与相机另运行 `node tests/research-connections-browser.js`，使用相同三个环境变量和隔离数据根；覆盖真实命中、创建/改名/删除及撤销、拖动提交/取消、移除引用保留对象关系、保存重开、名称合成 IME、第二窗口只读、定位/适应、黑白主题、窄窗及低动态。截图输出到临时 `relatum-research-links-*`，脚本结束关闭测试服务。

该浏览器回归也覆盖 Alt 拖线/取消、类型选择后双击创建、Tab/Enter 子/同级分支、树布局、整枝移动/取消/撤销、折叠保存重开与定位展开、移除中间呈现/脱离父级及只读禁用。模型回归另检查循环拒绝、未知父级、不等高布局和 12000 层迭代遍历；这不等于通过大场景渲染验收。Python 研究测试覆盖层级格式拒绝与往返持久化。

...
| `assets/study.js` | 独立学习任务系统：极简清单、单位进度面板、自适应数字任务页、每页可选说明、引用式临时任务侧栏、回收站与完成归档；任务数据与总路线共用。进度视图错峰入场中离页时冻结当前帧，外层退场后再清理。任务卡片颜色存在任务的 `color` 字段，进度卡/清单行/临时侧栏右键弹出同款 12 色调色盘。 |
...
| `assets/ledger.js`、`assets/ledger-model.js` | 日历页第二视图的极简人民币流水：无 DOM 模型层用 BigInt 定点运算统一金额解析、四位十进制倍率、半入舍入、筛选与汇总；页壳、日期组和账目卡使用常驻 Map 索引、渲染指纹与按 ID 增量同步。右缘页栏提供 1–99 虚拟数字页，月份在页间共享；账本一次全量加载后由本地按页/月派生，切页即时完成并保留未保存草稿。点击 `＋` 只创建当前页本地草稿；保存有效金额时编辑框立即关闭并把带稳定客户端 ID 的乐观账目放入列表，后端在后台落盘，失败时只撤回该乐观条目并恢复为可重试草稿。切月或离页丢弃尚未提交的草稿。写接口只回 revision 与目标增量；同账目请求串行，不同账目只合并各自响应，失败只回滚目标并在修订跳号或结果不确定时合并重载。账目卡使用并列的主编辑按钮和选项按钮，不嵌套交互控件。仍不包含预算、趋势、分类、搜索、筛选、批量、多账户、多币种或周期账单。 |
...
- 整本账本在首屏空闲 `requestIdleCallback` 预热，首次翻入直接消费；权威重载使用 `AbortController` 与递增序号。乐观操作只改目标条目或页单位，同目标请求串行，响应按增量合并；失败只回滚当前目标，revision 跳号或请求结果不确定时等待 mutation 清空后合并一次 GET。页视图与隐藏小数是本地偏好，单位未变时不得发 POST。汇总、行增删改、颜色、跨视图与月份动画均服从 `prefers-reduced-motion`。

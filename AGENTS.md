...
| `assets/study.js` | 独立学习任务系统：极简清单、单位进度面板、自适应数字任务页、每页可选说明、引用式临时任务侧栏、回收站与完成归档；任务数据与总路线共用。当前数字页的非空说明同时显示在进度视图主标题与极简清单 `To Do` 标题右侧，并共用字号、字重、颜色与双击编辑保存链。进度视图错峰入场中离页或切换右侧数字页时冻结当前帧，当前页整体退场后再清理并替换内容。任务卡片颜色存在任务的 `color` 字段，进度卡/清单行/临时侧栏右键弹出同款 12 色调色盘。 |

...
| `assets/desktop-shell.js` | 前端到 pywebview 的桥接、窗口按钮、dirty 标记；桌面关闭按钮支持注册异步保存前置钩子。笔记专注模式收起顶栏后，桌面壳在右上角提供浮动关闭按钮，与原关闭按钮共用保存前置钩子和关闭路径；连续点击只执行一次在途关闭。 |
...
排序与新建命名另用 `node tests/note-tree-sort-browser.js` 验证六种顺序、偏好恢复、元数据重排、自动编号和弹层操作；同样只连接隔离数据根。

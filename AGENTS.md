...
| `assets/canvas.js` | 核心画布引擎，节点/边/手写/附件/批注/选择/历史/导图/独立表格/AI 计划执行和双屏选区导入导出。左侧直线工具共用同一入口创建纯直线或终点箭头，两者都可在选中后添加转折点；新建对象在 `ink.arrows[]` 中用 `arrowhead:"none"|"end"` 显式保存端点类型。 |
...
- 窗口回焦和手动刷新始终读取最新文件树元数据、校验正文缓存；仅当路径、名称、类型、顺序或子级结构改变时重建文件树 DOM。仅修改时间/大小变化不重建行，保留树内重命名草稿、焦点和进行中的目录过渡；刷新失败显示错误后，下次成功必须恢复文件树，即使结构未变。`tests/note-view-state-browser.js` 覆盖无变化刷新零子节点增删、修改时间变化、外部增删、重命名焦点和错误后恢复。
...
- 笔记正文基线为 16px / 1.65 行高，最大有效行宽 880px，宽窗左边距最多 112px；正文和行内文件标题优先使用系统 UI 字体栈，Noto Sans SC / 微软雅黑只作跨机器后备，且禁用伪造字重。实时预览与阅读模式的代码块使用相对正文 .94em 字号、1.65 行高和轻中性底色；代码与源码模式共用带明确中文无衬线后备的 `--note-font-mono`，避免中文注释落到默认衬线字体。行内文件标题保持 32px，Markdown H1 为 1.75em 并逐级收敛，不能让 H1 与文件标题同大；齿轮中的百分比滑条只缩放正文与行内文件标题。文件路径栏保持固定，行内文件标题及其顶部占位属于正文滚动内容：滚动时通过共享编辑表面的 `scrollTop` 同步移出并裁切，不能常驻挤压正文。词数/字符数统计定位为正文右下角的无交互悬浮层，不得作为 flex 子项独占一行；字符数可用 CodeMirror 文档长度随输入即时更新，词数在 350ms 快照或其他既有保存边界更新，不能为了统计恢复逐键全文 `toString()`。
...
- 左侧书脊保留两层游标：黑白实体表示真实当前页，彩色细条与柔光层负责悬停预览；彩色层初载直接落在当前页，悬停时连续改道，离开书脊、焦点离开或滚动后回到当前页并常驻为细条 + 微光轮廓，不得重新改成淡出消失。深色主题的书脊底板使用均匀、不透明的中性黑，不叠加环境色或局部彩色光晕。动态分组重建和窗口缩放必须无飞入地校准，低动态模式立即完成位置与形状切换。
...
编辑位置记忆另用 `node tests/note-view-state-browser.js` 验证；测试主机设置 `RELATUM_PLAYWRIGHT`、`RELATUM_PYTHON`、`RELATUM_EDGE_PATH` 后，脚本自动启动隔离数据根与真实笔记 API，覆盖自身保存后刷新、失焦/回焦事件、外部修改、关闭重开、缓存淘汰、反向选区、页面重载与新浏览器上下文、文件/文件夹改名及失败回滚、正文缩短、三种视图和位置偏好的数量上限/损坏/写入失败；结束时关闭测试服务。这不替代用户电脑上 WebView2 与其他桌面软件之间的实际切换验收。

实时预览排版与命中用 `node tests/note-live-presentation-browser.js` 验证；设置上述 Playwright 与 Edge 路径即可，不需要 Python。脚本只托管静态资产和合成笔记，在临时目录输出深浅截图与 JSON 报告，覆盖长代码块可见装饰数量、重复 Callout/表格/分隔线下方代码的真实鼠标点击、窄窗与 135% 字号、中文 IME、模式切换后的撤销、任务框和未闭合围栏复制，并复用长文档视口/公式测试与 4MiB 文档输入耗时探针；不接触真实笔记库。

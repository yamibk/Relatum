...
'把任务长度拆成有名字的任务点。达成状态会根据当前进度自动点亮。': 'Break the target into named milestones. Status lights up automatically as you progress.',
    ...
    '目标、阶段、任务与连线': 'Goals, stages, tasks, and links', '进度怎样计算': 'How progress is calculated',
    '更快地编辑路线': 'Edit routes faster',
    '目标是整棵路线；阶段用来收纳一组学习任务，本身没有手动进度。': 'The goal is the whole route. A stage groups study tasks and has no manual progress of its own.',
    '浅灰无箭头线：阶段“包含”任务。': 'Light gray line without an arrow: a stage contains a task.',
    '深色箭头线：完成来源后，解锁下一项；虚线表示附加条件。': 'Dark arrow: completing the source unlocks the next item; a dashed line is an additional condition.',
    '所有任务等权：已完成是 100%；有总量时按“当前 ÷ 总量”；未设置总量且未完成是 0%。': 'Every task has equal weight: completed is 100%; a task with a target uses current ÷ target; an incomplete task without a target is 0%.',
    '阶段只平均它包含的任务，根目标平均整棵树中的唯一任务。点击阶段或根目标的进度文字，可以查看逐项公式。': 'A stage averages only its contained tasks; the root goal averages every unique task in the tree. Click its progress text to see the formula.',
    '一个节点有多个条件时采用 AND 规则：必须全部满足才可推进。主路线条件决定树的排版位置；附加条件只增加约束，不改变排版和进度归属。': 'Multiple conditions use AND: every condition must be met. The primary route condition controls layout; additional conditions add constraints without changing layout or progress ownership.',
    '被锁定的任务仍可改名、查看条件或移出路线，但不能完成或修改进度。': 'A locked task can still be renamed, inspected, or removed from the route, but its completion and progress cannot change.',
    '拖动节点可调整主路线；“下一步”会依次定位当前可开始的任务；阶段可折叠，多棵目标树从左侧 Rail 切换。': 'Drag nodes to edit the primary route. Next locates available tasks in order. Stages can collapse, and the left rail switches between goal trees.',
    '起步页齿轮里的“精简目标树编辑”默认开启。关闭后，才显示新建后续阶段、添加解锁条件等高级入口；切换不会改动已有数据。': 'Simplified Goal Tree editing is on by default in Home settings. Turn it off to show advanced creation controls; switching modes never changes existing data.',

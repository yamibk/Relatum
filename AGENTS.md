...
- 图片文字只属于独占行本地图片。选中图片后右上角“图片文字”入口才启用；进入模式后可一次性放置、选择、拖动、双击纯文本编辑、整框改四档字号/固定字色或删除，切源码/阅读、换笔记、取消图片选择或按 `Esc` 自动退出。位置保存为图片内部中心点比例，字号按显示宽度的 `2.5%/3.5%/5%/7%` 计算；最多 64 框、每框 1000 字符、解码 JSON 最大 48KiB。有效 v1 数据写在图片语法后的 Base64URL JSON 注释中；删除最后一框移除整段注释。未知版本、损坏或超限数据原样只读保留，远程与行内图片不启用。阅读/Live 都用 `textContent` 或已转义 HTML 显示；“合并为图片”当前是无监听、无请求、无写盘的占位按钮。
...
实时预览排版与命中用 `node tests/note-live-presentation-browser.js` 验证；设置上述 Playwright 与 Edge 路径即可，不需要 Python。脚本只托管静态资产和合成笔记，在临时目录输出深浅截图与 JSON 报告，覆盖长代码块可见装饰数量、重复 Callout/表格/分隔线下方代码的真实鼠标点击、窄窗与 135% 字号、中文 IME、模式切换后的撤销、任务框和未闭合围栏复制，并复用长文档视口/公式测试与 4MiB 文档输入耗时探针；图片文字还要人工或在具备 Playwright 的环境覆盖多框创建、IME、拖动边界、字号/字色、缩放、撤销重做、保存重开、Live/源码/阅读往返、深浅主题、窄窗、低动态，以及“合并为图片”点击前后 Markdown/撤销栈/请求/`.assets` 均不变。不接触真实笔记库。

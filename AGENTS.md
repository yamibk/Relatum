...
| `assets/ledger.js`、`assets/ledger-model.js` | 日历页第二视图的极简人民币流水：无 DOM 模型层用 BigInt 定点运算统一金额解析、四位十进制倍率、半入舍入、筛选与汇总；页壳、日期组和账目卡使用常驻 Map 索引、渲染指纹与按 ID 增量同步。右缘页栏提供 1–99 虚拟数字页，月份在页间共享；账本一次全量加载后由本地按页/月派生，切页即时完成并保留未保存草稿。每页可在当前页设置中独立记忆新建账目的默认“支出/收入”，未设置时默认支出；点击 `＋` 只按该偏好创建当前页本地草稿。保存有效金额时编辑框立即关闭并把带稳定客户端 ID 的乐观账目放入列表，后端在后台落盘，失败时只撤回该乐观条目并恢复为可重试草稿。切月或离页丢弃尚未提交的草稿。写接口只回 revision 与目标增量；同账目请求串行，不同账目只合并各自响应，失败只回滚目标并在修订跳号或结果不确定时合并重载。账目卡使用并列的主编辑按钮和选项按钮，不嵌套交互控件。仍不包含预算、趋势、分类、搜索、筛选、批量、多账户、多币种或周期账单。 |
...
| `assets/focus.js` | 专注钟、正计时/番茄钟、每日任务绑定、音效/噪音、记录编辑。每日任务视图错峰入场中离页时冻结当前帧，外层退场后再清理；计时器视图保持原有退场行为。 |
...
- 右侧编号栏支持滚轮前后切树；树较多、编号列表可滚动时，先滚动栏内列表，到边缘后继续滚才切换。快速滚轮采用“滑块即时追随 + 首个目标立即切换 + 在途时只保留最新目标”的双通道：可任意跨过多页，不等待手势停稳；树状态仍按单请求串行落盘，但旧树退到透明交换点后，最终目标可从当前尚未结束的入场帧连续接管，不再等待完整入场，中间页也不逐页排队。接管必须继承当前透明度和水平位移，并按剩余可见度缩短新一轮退场，不能先恢复默认样式造成跳帧。编号栏 DOM 重建时先将黑色滑块无动画钉在正确起点，不得短暂回到第 1 页再跳回。

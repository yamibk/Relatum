...
| `assets/ledger.js` | 日历页第二视图的极简人民币流水：页壳、汇总、日期组和账目卡使用常驻 DOM 与按 ID 增量同步，增删改、换色和排序变化复用学习页的入退场、锚定设置浮层与 FLIP。右缘悬停页栏提供 1–99 虚拟数字页，月份在页间共享；账本一次全量加载后由本地按页/月派生当前视图与汇总（照搬学习页数据模型），切页即时完成、保留未保存草稿，不提供页命名、颜色、删除或账目跨页移动。点击 `＋` 只创建一张显示 `¥—` 的当前页本地草稿，保存有效金额后才落盘，切页保留（草稿跟随创建页），切月或离页丢弃；账目仍按日期/创建时间倒序，不提供拖拽排序。每笔可选正数倍率，留空按 1 计算；填写后账目行显示“原金额 × 倍率 = 最终金额”，月汇总使用最终金额。运行时在起步页首屏空闲期即预取整本账本，首次翻入记账视图直接消费内存快照。支持同槽二次确认删除、未来日期、跨月保存定位、共享 4×3 的 12 色标记、请求乱序保护与乐观更新整本回滚；不包含预算、趋势、分类、搜索、筛选、批量、多账户、多币种或周期账单。 |
...
- 日历、记账与倒数日：`/api/calendar`、`/api/ledger`、`/api/countdown`；记账 GET 不带参数，一次返回整本账本快照 `{version, highestPage, pageUnits, entries}`，流水按日期/创建时间倒序，页面/月份/累计汇总与当页单位由前端本地派生（与学习页同款模型），三个金额汇总均使用应用可选倍率后的最终分值。
- 模板：`/api/templates`
...
- 记账：`/api/ledger-entry-create`、`/api/ledger-entry-update`、`/api/ledger-entry-delete`、`/api/ledger-page-unit`；创建可传 1–99 的 `ledgerPage`，更新不支持跨页移动；更新既支持收入/支出、金额、可选倍率、日期和可选备注的完整表单，也支持单独颜色补丁，传入 `multiplier:null` 会清除倍率。每页单位接口只接受 `page/unit`，空 `unit` 恢复人民币。所有写响应都携带整本账本快照 `ledger`，前端整体替换本地数据（学习页 `applyStudyPayload` 同款模式）。
...
- 右侧边缘悬停或键盘聚焦显示 1–99 数字页栏，复用学习页按钮、活动滑块、隐藏滚动条和滚轮边缘切页手感；页栏至少铺满当前可用高度，并覆盖当前页及 `highestPage`。当前页写入 `ledger:page:v1`，切页保留月份与未保存草稿（草稿跟随创建页）并关闭设置/调色盘；切页照搬学习页 `setCurrentTaskPage`：全量数据本地直接过滤，150ms 淡出后隐藏态替换内容，再整版错峰入场，快速连点只让最后一次生效。
...
- 整本账本只请求一次：首屏空闲 `requestIdleCallback` 预热，首次翻入或后续加载复用同一个 promise；主请求使用 `AbortController` 与递增序号，旧响应不得覆盖新数据。乐观增删改先改本地全量、失败整本回滚，成功后以响应快照对齐。汇总、行增删改、颜色、跨视图与月份动画均服从 `prefers-reduced-motion`，离页时清理。
...
并使用隔离的 `RELATUM_DATA_ROOT` 验证本月空状态、未来月份新增、跨月修改、同槽删除确认、全量加载后本地切页即时切换与草稿保留、请求乱序保护、窄窗、键盘、深浅主题和低动态；结束后关闭测试服务，不得读取或写入现有 `data/ledger.json`。

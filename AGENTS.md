...
- `researchVersion:2` 不兼容 V1：非 V2 研究文件读取为空白 V2，首次保存直接覆盖且不备份 V1。节点为 `{id,type,label,x,y,width,height,config,statePolicy,savedState?}`；边只能是 `relation` 或显式端口 `wire`。关系线不执行；值输入单连接、事件输入多来源、输出可扇出。建线时即时拒绝错误方向、value/event 通道、声明类型、已知 Bits 位宽、同组输入不匹配和占用，不得先写历史再回滚。

...
const obsidianSized = Markdown.parseImage('  ![[photo.webp|300x200]]  ');
assert.strictEqual(obsidianSized.syntax, 'wiki');
assert.strictEqual(obsidianSized.target, 'photo.webp');
assert.strictEqual(obsidianSized.width, 300);
assert.strictEqual(obsidianSized.height, 200);
assert.strictEqual(Markdown.serializeImage(obsidianSized, { width: 420 }), '  ![[photo.webp|420]]  ');
assert.strictEqual(Markdown.serializeImage(obsidianSized, { width: 420, height: 280 }), '  ![[photo.webp|420x280]]  ');
const markdownSized = Markdown.parseImage('![Diagram|360](<page assets/diagram.png> "Overview")');
assert.strictEqual(markdownSized.syntax, 'markdown');
assert.strictEqual(markdownSized.alt, 'Diagram');
assert.strictEqual(markdownSized.target, 'page assets/diagram.png');
assert.strictEqual(markdownSized.title, 'Overview');
assert.strictEqual(markdownSized.width, 360);
assert.strictEqual(Markdown.serializeImage(markdownSized, { width: 480 }),
  '![Diagram|480](<page assets/diagram.png> "Overview")');
const numericAltSize = Markdown.parseImage('![250](photo.webp)');
assert.strictEqual(numericAltSize.alt, '');
assert.strictEqual(numericAltSize.width, 250);
const percentAlias = Markdown.parseImage('![[photo.webp|50%]]');
assert.strictEqual(percentAlias.alt, '50%');
assert.strictEqual(percentAlias.width, null);
const aliasedWiki = Markdown.parseImage('![[photo.webp|封面|300]]');
assert.strictEqual(aliasedWiki.alt, '封面');
assert.strictEqual(aliasedWiki.width, 300);
assert.strictEqual(Markdown.serializeImage(aliasedWiki, { width: 420 }), '![封面|420](photo.webp)');
const renderedSize = Markdown.render('![[photo.webp|300x200]]', { localImages: true });
assert(renderedSize.includes('class="md-local-image has-explicit-size has-explicit-box"'));
assert(renderedSize.includes('style="width:min(100%,300px);aspect-ratio:300/200"'));
assert(!Markdown.render('![[photo.webp|50%]]', { localImages: true }).includes('has-explicit-size'));

...
| `assets/career-report.js` | 起步页「生涯」冻结使用报告适配层；首次进入时按需加载，只读 `data/career-report.json` 快照并用原生 HTML/CSS/SVG 绘制使用概况、画布、笔记、学习、使用习惯与数据范围六章。“使用习惯”复用已有快照展示月度画布时长、当前笔记最后修改月份、双链/孤立笔记以及打卡/日记日期；专注和复习仍保留在快照与数据来源状态中，但不占主要图表。进入生涯后全局顶栏自动收至窗口上沿，鼠标移入顶部感应区或键盘焦点进入时临时展开。滚动不拦截滚轮；章节进入视口时，统计数字从 0 递增，折线、细线、柱形、点阵、热力格与关系图按有限错峰节奏展开，并有 `prefers-reduced-motion` 静态降级。图表可悬停/键盘聚焦，但数值只能由页底“重新生成”替换。不调用 AI、不联网、不保存正文或绝对路径。 |
